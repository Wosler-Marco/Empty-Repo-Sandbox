...
class WebSocketClient : public std::enable_shared_from_this<WebSocketClient> {
	private:
		...
		void on_resolve(beast::error_code ec, tcp::resolver::results_type results) {
			...
			beast::get_lowest_layer(ws).async_connect(results, beast::bind_front_handler(&WebSocketClient::on_connect, shared_from_this()));
		}

		/**
		 * @brief Method to asynchronously perform operations to set options for the WebSocket and perform the handshake after the connection is made
		 * 
		 * @param ec An error code object containing both a code and a text description of the specific error
		...
		void on_connect(beast::error_code ec, tcp::resolver::results_type::endpoint_type ep) {
			...
			// Turn off the timeout on the tcp_stream since Websocket has its own timeout
			...
			ws.async_handshake(host, "/", beast::bind_front_handler(&WebSocketClient::on_handshake, shared_from_this()));
		}
		...
		void on_handshake(beast::error_code ec) {
			...
			ws.async_read(buffer, beast::bind_front_handler(&WebSocketClient::on_read, shared_from_this()));
		}
		...
		void on_write(beast::error_code ec, std::size_t bytes_transferred) {
			...
			if (writeQ.empty()) {
				...
				ws.async_write(net::buffer(writeQ.front()), beast::bind_front_handler(&WebSocketClient::on_write, shared_from_this()));
			}
		}
	...
	public:
		 ...
		 */
		explicit WebSocketClient(net::io_context& ioc) : resolver(net::make_strand(ioc)), ws(net::make_strand(ioc)) {
			connected = false;
		}
		 ...
		 */
		void run(std::string target, std::string port) {
			host = target;

			// Look up the domain name
			resolver.async_resolve(host, port, beast::bind_front_handler(&WebSocketClient::on_resolve, shared_from_this()));
		}
		...
		void close() {
            ...
            ws.async_close(websocket::close_code::normal,beast::bind_front_handler(&WebSocketClient::on_close, shared_from_this()));
        }
};
