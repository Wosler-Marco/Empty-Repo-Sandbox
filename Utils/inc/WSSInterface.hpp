...
class WSSInterface : public std::enable_shared_from_this<WSSInterface> {
	private:
	...
	public:
		 ...
		 */
		int connect(std::string const& host, std::string port = "443") {
			std::string uri = "wss://" + host + ":" + port;
		...
		void send(int id, std::string message, websocketpp::frame::opcode::value type = websocketpp::frame::opcode::text) {
			websocketpp::lib::error_code ec;
			...
			m_endpoint.send(metadata_it->second->get_hdl(), message, type, ec);
			if (ec) {
