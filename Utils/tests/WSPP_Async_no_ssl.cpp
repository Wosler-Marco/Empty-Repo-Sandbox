...
std::mutex cout_mut;
std::mutex ws_mut;
wosler::utilities::WSInterface endpoint;
...
public:
    ...
    virtual void OnProcess() override
    {
        if(_step == Login) {
            ...
            _id = endpoint.connect("sonolink.brightersolutions.ca");
            std::cout << "Endpoint created: " << _id << "\n";
            ...
            // Send the message
            try{
                // std::vector<uint8_t> packedData = nlohmann::json::to_cbor(messageJSON);
                // int id = _id;
                // std::cout << id << "\n\n";
                // endpoint.send(_id,std::string(packedData.begin(), packedData.end()));
                auto packedData = nlohmann::to_string(messageJSON);
                endpoint.send(_id,std::string(packedData.begin(), packedData.end()));
