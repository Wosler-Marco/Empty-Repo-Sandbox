                                    ...
                                    PSampleStreamingSession *ppSampleStreamingSession)
{
    ...
    videoRtpTransceiverInit.direction = RTC_RTP_TRANSCEIVER_DIRECTION_SENDRECV; // this means peer can send and receive
...
/**
 * Callback when a signaling message is received.
 *
 * This function processes the received signaling message and handles different message types
 * such as offers, answers, and ICE candidates.
 *
 * @param customData - Custom data passed to the callback
 * @param pReceivedSignalingMessage - Pointer to the received signaling message
 */
STATUS signalingMessageReceived(UINT64 customData, PReceivedSignalingMessage pReceivedSignalingMessage)
{
    ...
    MUTEX_LOCK(pSampleConfiguration->sampleConfigurationObjLock);
    locked = TRUE;
    // all peers store the clientid in hashtable
